/* oxlint-disable typescript/no-explicit-any, unicorn/switch-case-braces -- byte-identical port of the legacy WalletConnect v1 connector; kept as-is for wire compatibility */
import type {
  IClientMeta,
  IConnector,
  IConnectorOpts,
...
class Connector implements IConnector {
  ...
  public async connect(opts?: ICreateSessionOptions): Promise<ISessionStatus> {
    ...
    // oxlint-disable-next-line no-async-promise-executor -- legacy behavior preserved verbatim
  ...
  private async _handleIncomingMessages(socketMessage: ISocketMessage) {
    ...
    try {
      ...
      // oxlint-disable-next-line no-unused-vars -- legacy behavior preserved verbatim
    } catch (error) {
      return;
    }

    const payload: IJsonRpcRequest | IJsonRpcResponseSuccess | IJsonRpcResponseError | null =
      await this._decrypt(encryptionPayload);
  ...
  private async _decrypt(
    ...
    const key: ArrayBuffer | null = this._key;
    if (this._cryptoLib && key) {
      const result: IJsonRpcRequest | IJsonRpcResponseSuccess | IJsonRpcResponseError | null =
        await this._cryptoLib.decrypt(payload, key);
      return result;
    }
    return null;
  }
