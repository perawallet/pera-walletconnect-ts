...
class EventManager {
  ...
  public trigger(
    ...
    if (
      ...
      // upstream checked isInternalEvent(event) which is always false for strings
      // oxlint-disable-next-line typescript/no-explicit-any -- legacy behavior preserved verbatim
      !isInternalEvent(event as any)
    ) {
      eventEmitters = this._eventEmitters.filter(
